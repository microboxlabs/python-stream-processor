...
from ..utils.metrics import segments_generated_total, ffmpeg_duration_histogram
...
class HLSGenerator:
    ...
    def __init__(self):
        ...
        Path(self.storage.hls_path).mkdir(parents=True, exist_ok=True)

    def _get_device_hls_path(self, device_id: str) -> Path:
        """Get HLS output path for a device."""
        device_path = Path(self.storage.hls_path) / device_id
    ...
    def generate_segment(
        ...
        device_path = self._get_device_hls_path(device_id)
        ...
        try:
            ...
            if not segment_path.exists():
                raise RuntimeError(f"FFmpeg succeeded but segment not found: {segment_path}")
            ...
            # Update playlist
            self._update_playlist(device_id, segment_number, segment_filename)
            
            return str(segment_path)
            
        except subprocess.CalledProcessError as e:
            logger.error(f"FFmpeg failed: {e.stderr}")
            raise RuntimeError(f"FFmpeg generation failed: {e.stderr}")
    ...
    def get_playlist_path(self, device_id: str) -> Path:
        """Get the playlist path for a device."""
        return self._get_device_hls_path(device_id) / "playlist.m3u8"

    def get_segment_path(self, device_id: str, segment_filename: str) -> Path:
        """Get a segment file path for a device."""
        return self._get_device_hls_path(device_id) / "segments" / segment_filename
